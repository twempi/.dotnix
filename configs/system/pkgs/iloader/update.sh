...
pkg_file="$repo_root/configs/system/pkgs/iloader/default.nix"
api_url="https://api.github.com/repos/nab138/iloader/releases/latest"
asset_name="iloader-linux-amd64.deb"
...
export ILOADER_VERSION="$version"
export ILOADER_HASH="$hash"
perl -0pi -e 's/(version = ")[^"]+(";\n)/$1$ENV{ILOADER_VERSION}$2/' "$pkg_file"
perl -0pi -e 's/(hash = ")[^"]+(";\n)/$1$ENV{ILOADER_HASH}$2/' "$pkg_file"

echo "Updated iloader to $version"
